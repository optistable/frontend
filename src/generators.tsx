...
import {DataProvider, OracleCommittee, Policy} from "./Home";
...
export const generateHexString = (length: number): string => {
    ...
    const mixedCaseResult = result.split('').map(char => Math.random() < 0.5 ? char.toLowerCase() : char).join('');

    return mixedCaseResult;
}
